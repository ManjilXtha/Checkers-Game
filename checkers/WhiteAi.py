import pygame
from checkers.constant import RED, WHITE, YELLOW, SQUARE_SIZE,HEIGHT,WIDTH,DOUBLE_JUMP_SOUND, CAPTURE_SOUND, MOVE_SOUND, KING_SOUND,SERRIA,ROPE,BOARD_HEIGHT,BOARD_WIDTH
import pygame.mixer
...
class PVWhite:
    ...
    def _move(self, row, col):
        ...
        if self.selected and piece == 0 and (row, col) in self.valid_moves:
            ...
            if_promotion=self.board.if_promotion(self.selected,row)
            if if_promotion:
                KING_SOUND.play()
            elif skipped:
                if len(skipped) > 1:
    ...
    def run_pvred(self):
        ...
        while run:
            ...
            if self.turn == RED:
                value, new_board = minimax(self.get_board(), 4, RED, self,-99,99)
            ...
            for event in pygame.event.get():
                ...
                if not pause and not confirm:
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        pos = pygame.mouse.get_pos()
                        #retry
                        retry_x = BOARD_WIDTH + (WIDTH - BOARD_WIDTH) // 2 - 25
                        retry_y = HEIGHT // 2 - 30
                        retry_size = 55
                        retry_rect = pygame.Rect(retry_x, retry_y, retry_size, retry_size)
                        #pause
                        pause_x = BOARD_WIDTH + (WIDTH - BOARD_WIDTH) // 2 - 25
                        pause_y = HEIGHT // 2 - 90
                        pause_size = 55
                        pause_rect = pygame.Rect(pause_x,pause_y,pause_size, pause_size)
                        #home
                        home_rect = pygame.Rect(BOARD_WIDTH + (WIDTH - BOARD_WIDTH) // 2 - 25,
                                                HEIGHT // 2 + 30,
                                                55,55
                        )
